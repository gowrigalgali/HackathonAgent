...
export default function IdeaStudioPage() {
  ...
  useEffect(() => {
    setMounted(true);
    // Load persisted ideas from localStorage
    const savedIdeas = localStorage.getItem('hackathon-ideas');
    const savedContent = localStorage.getItem('hackathon-result');
    if (savedIdeas) {
      try {
        const parsedIdeas = JSON.parse(savedIdeas);
        setIdeas(parsedIdeas);
        console.log('Loaded saved ideas:', parsedIdeas);
      } catch (e) {
  ...
  useEffect(() => {
    if (ideas.length > 0 && mounted) {
      localStorage.setItem('hackathon-ideas', JSON.stringify(ideas));
      console.log('Saved ideas to localStorage:', ideas);
    }
  }, [ideas, mounted]);
  ...
  const generateIdeas = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to generate ideas:', errorData);
      }
    } catch (error) {
  ...
  const createGitHubRepo = async (idea: Idea) => {
    try {
      // Create a repository name from the idea title
      const repoName = idea.title.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
      ...
      const response = await fetch('/api/create-github-repo', {
        ...
        body: JSON.stringify({
          ...
          files: [
            {
              path: 'README.md',
              content: `# ${idea.title}\n\n${idea.pitch}\n\n## Tech Stack\n\n${idea.tech}\n\n## What Makes It Unique\n\n${idea.novelty}\n\n## Getting Started\n\nThis is a hackathon project generated by HackathonAgent.\n\n## Next Steps\n\n1. Clone this repository\n2. Set up your development environment\n3. Start building your amazing project!\n\nHappy coding! 🚀`
            }
          ],
          private: false
        }),
        ...
        >
          <Card className="p-6 border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20">
            ...
            <Button 
              ...
              variant="outline"
              className="rounded-xl border-red-300 text-red-700 hover:bg-red-100 dark:border-red-700 dark:text-red-300 dark:hover:bg-red-900/30"
            >
              Dismiss
            </Button>
          </Card>
